...
class MaterialParams:
    ...
    # Optional provenance line for the header comment (source / asset / license).
...
def generate(params: MaterialParams) -> str:
    ...
    if params.source_note:
        lines.append(f"# source: {params.source_note}")
    lines += [
        "",
        # pic_u/pic_v are the aspect-scaled picture coordinates from the .cal
        # (Radiance spans a picture's long side over 0..aspect, not 0..1);
        # the texdata/brightdata .dat modifiers below keep the unit-square u v.
        f"void colorpict {pat}",
        f"7 red green blue {params.hdr_file} {params.cal_file} pic_u pic_v",
        "0",
        "0",
        "",
    ]
